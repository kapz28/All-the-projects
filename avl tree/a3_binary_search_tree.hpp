...
public:
  ...
  struct Node {
    ...
    int avlBalance;   // used for AVL trees
  };
...
public:
  ...
  //set root node to point to a node
  void setRootNode(Node* kap);
  
  
  //used to increase size
  void increasesize();
  //used to decrease size
  void decreasesize();
  //my own helper function used to track predecessor/sucessor
  Node* predecessor_sucessor(Node* track);
  //my own helper function used for remove
  //if value doesn't exist then does nothing
  //if value exists return the node
  //also changes the parent node to the parent node
  Node* search(DataType val,Node** parent);
