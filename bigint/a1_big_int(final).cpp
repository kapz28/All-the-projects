...
//	sum->a = new DoublyLinkedList();
	...
	//		cout << "strNew" << strNew << endl;
		...
		//unsigned int value = bin2dec(strNew);
		//cout << "value" << value << endl;
		
		//sum->a->insert(value,0);
	}
	if (prevNum == 1){
		strNew = '1' + strNew;
	}
...
BigInt* BigInt::mul(BigInt* rhs) 
{
	...
	/*rhs->a->size()-1)*/
	cout<<"size of rhs is" <<rhs->a->size();
	while(rhs->a->select(rhs->a->size()-1)!=0){
		cout<<"thing is"<<(rhs->a->select(0));
		rhs->sub(d);
		cout<<"subtracted is"<<rhs->a->select(0);
		add(this);
	}
	a->print();
}
